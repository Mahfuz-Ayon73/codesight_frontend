...
export async function GET(request: NextRequest) {
  ...
  const res = await fetch(
    `${API_BASE}/api/v1/organizations/${organizationId}/projects/${projectId}/graph/overrides?snapshotId=${snapshotId}`,
...
export async function PUT(request: NextRequest) {
  ...
  const res = await fetch(
    `${API_BASE}/api/v1/organizations/${organizationId}/projects/${projectId}/graph/overrides/cluster`,
    {
      method: "PUT",
