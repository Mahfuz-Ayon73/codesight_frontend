...
export default async function OrgProjectsPage({ params }: Props) {
  ...
  try {
    ...
    loadError = e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Failed to load projects";
  }
          ...
          {projects.map((project) => (
            ...
            >
              <div className="flex flex-col gap-0.5 min-w-0">
                ...
                {project.description && (
                  <p className="text-xs text-zinc-400 max-w-xs truncate">{project.description}</p>
                )}
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <div className="flex items-center gap-1 text-xs text-zinc-400">
                  <Clock size={11} />
                  {project.uploadedAt ? formatDate(project.uploadedAt) : "Not uploaded"}
                </div>
                <StatusBadge status={project.analysisStatus} />
                <Link
                  href={`/organizations/${organizationId}/projects/${project.id}/details`}
